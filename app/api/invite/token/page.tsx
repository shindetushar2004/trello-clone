...
//   useEffect(() => {
//     // Pehle login check karo
//     if (!isLoaded) return;

//     if (!isSignedIn) {
//       // Login page pe redirect karo, wapas yahan aane ke liye
...
//           <p className="text-gray-600">Board join ho raha hai...</p>
//         </div>
//       </div>
//     );
//   }
...
export default function BoardPage() {
  ...
  async function handleDragEnd(event: DragEndEvent) {
    ...
    if (targetColumn) {
      ...
      const sourceColumn = columns.find((col) => col.tasks.some((task) => task.id === taskId));
      const targetCol = columns.find((col) => col.tasks.some((task) => task.id === overId));

      if (sourceColumn && targetCol) {
        const oldIndex = sourceColumn.tasks.findIndex((task) => task.id === taskId);
        const newIndex = targetCol.tasks.findIndex((task) => task.id === overId);
        if (oldIndex !== newIndex) {
          await moveTask(taskId, targetCol.id, newIndex);
        }
      }
    }
  }
          ...
          <DialogContent className="w-[95vw] max-w-[425px] mx-auto">
            ...
            <form className="space-y-4" onSubmit={handleUpdateBoard}>
              ...
              <div className="space-y-2">
                <Label>Board Color</Label>
                <div className="grid grid-cols-4 sm:grid-cols-6 gap-2">
                  {["bg-blue-500","bg-green-500","bg-yellow-500","bg-red-500","bg-purple-500","bg-pink-500","bg-indigo-500","bg-gray-500","bg-orange-500","bg-teal-500","bg-cyan-500","bg-emerald-500"].map((color, key) => (
                    <button
                      key={key}
                      type="button"
          ...
          {inviteStatus === "sent" ? (
            ...
            <div className="text-center py-6">
              ...
              <p className="text-sm text-gray-600 mb-4">
                They will receive an invite link via email.
              </p>
            ...
            <form className="space-y-4" onSubmit={handleSendInvite}>
              ...
              <div className="bg-gray-50 rounded-lg p-3 flex items-center gap-2">
                <div className={`w-3 h-3 rounded-full ${board?.color || "bg-blue-500"}`} />
                <span className="text-sm text-gray-700 font-medium">{board?.title}</span>
              </div>
